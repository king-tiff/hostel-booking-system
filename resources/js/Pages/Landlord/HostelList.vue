                ...
                <tr v-for="(hostel, index) in hostels" :key="index">
                    ...
                    <td class="border px-4 py-2">
                        <div class="flex items-center space-x-4 text-sm">
                            <Link
                                :href="route('landlord.hostel.show', hostel.id)"
                            ...
                            <Link
                                :href="route('landlord.hostel.edit', hostel.id)"
                                class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-main-primary rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                aria-label="Edit"
                            >
                                <svg
                            ...
                            <Button
                                type="danger"
                                @click="showConfirmationModal(hostel.id)"
                                class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-main-primary rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                aria-label="Delete"
                            >
                                <svg
...
const deletehostelPost = (hostel) => {
    form.delete(route("landlord.hostel.destroy", hostel), {
        preserveScroll: true,
